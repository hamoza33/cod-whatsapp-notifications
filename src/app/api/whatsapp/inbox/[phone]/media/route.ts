import { NextRequest, NextResponse } from "next/server";
import { getAuthUser } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { WhatsAppClient, WhatsAppApiError } from "@/lib/whatsapp";
import { getSetting, SETTING_KEYS } from "@/lib/settings";
import { rateLimit } from "@/lib/rate-limit";

/**
 * Upload a media file to Meta's Cloud API and send it as a chat message to
 * the customer in a single call. Used by the inbox's paperclip button so
 * the operator can send images, videos, audio, and documents inline.
 *
 * Expects `multipart/form-data` with:
 *   - `file`     (required) — the binary upload
 *   - `caption`  (optional) — text caption to attach (image / video / document only)
 *   - `numberId` (optional) — DB id of the WhatsappNumber to send FROM
 *
 * Only valid inside Meta's 24-hour customer service window — outside the
 * window Meta refuses the send with 131047 and we surface a friendly error.
 */
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ phone: string }> }
) {
  const user = getAuthUser(request);
  if (!user) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const { allowed } = rateLimit(`whatsapp-inbox-media:${user.id}`, 30, 60_000);
  if (!allowed) {
    return NextResponse.json(
      { error: "Rate limit exceeded for media sends." },
      { status: 429 }
    );
  }

  const { phone } = await params;
  const decodedPhone = decodeURIComponent(phone);

  let form: FormData;
  try {
    form = await request.formData();
  } catch {
    return NextResponse.json(
      { error: "Expected multipart/form-data with a 'file' field." },
      { status: 400 }
    );
  }

  const file = form.get("file");
  if (!(file instanceof Blob)) {
    return NextResponse.json(
      { error: "'file' field is required and must be a file." },
      { status: 400 }
    );
  }
  const caption =
    typeof form.get("caption") === "string"
      ? (form.get("caption") as string).trim()
      : "";
  const numberId =
    typeof form.get("numberId") === "string"
      ? (form.get("numberId") as string).trim()
      : "";

  const fileType = file.type || "application/octet-stream";
  const filename = file instanceof File ? file.name : "upload";

  // Determine the WhatsApp message type from the file MIME.
  let mediaType: "image" | "video" | "audio" | "document";
  if (/^image\//.test(fileType)) mediaType = "image";
  else if (/^video\//.test(fileType)) mediaType = "video";
  else if (/^audio\//.test(fileType)) mediaType = "audio";
  else mediaType = "document";

  // Resolve which WABA we're sending from.
  let phoneNumberIdOverride: string | null = null;
  if (numberId) {
    const numberRow = await prisma.whatsappNumber.findUnique({
      where: { id: numberId },
    });
    if (numberRow) {
      phoneNumberIdOverride = numberRow.phoneNumberId;
    } else if (/^\d{6,20}$/.test(numberId)) {
      phoneNumberIdOverride = numberId;
    }
  }

  let client: WhatsAppClient;
  try {
    client = await WhatsAppClient.fromSettings({ phoneNumberIdOverride });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "WhatsApp not configured" },
      { status: 400 }
    );
  }

  const accessToken =
    (await getSetting(SETTING_KEYS.WHATSAPP_ACCESS_TOKEN)) ||
    process.env.WHATSAPP_ACCESS_TOKEN ||
    "";
  const apiVersion =
    (await getSetting(SETTING_KEYS.WHATSAPP_API_VERSION)) ||
    process.env.WHATSAPP_API_VERSION ||
    "v17.0";

  // Step 1: upload to /media on the sending PNI.
  const metaForm = new FormData();
  metaForm.set("messaging_product", "whatsapp");
  metaForm.set("type", fileType);
  metaForm.set("file", file, filename);

  const uploadUrl = `https://graph.facebook.com/${apiVersion}/${client.getPhoneNumberId()}/media`;
  let uploadResp: Response;
  try {
    uploadResp = await fetch(uploadUrl, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}` },
      body: metaForm,
    });
  } catch (err) {
    return NextResponse.json(
      { error: `Network error uploading media: ${err instanceof Error ? err.message : "unknown"}` },
      { status: 502 }
    );
  }
  const uploadText = await uploadResp.text();
  if (!uploadResp.ok) {
    return NextResponse.json(
      { error: `Media upload failed (${uploadResp.status}): ${uploadText.slice(0, 400)}` },
      { status: uploadResp.status }
    );
  }
  let uploaded: { id?: string };
  try {
    uploaded = JSON.parse(uploadText) as { id?: string };
  } catch {
    return NextResponse.json(
      { error: `Non-JSON media upload response: ${uploadText.slice(0, 300)}` },
      { status: 502 }
    );
  }
  if (!uploaded.id) {
    return NextResponse.json(
      { error: `Media upload response missing id: ${uploadText.slice(0, 300)}` },
      { status: 502 }
    );
  }

  // Step 2: send the message referring to the freshly-uploaded media_id.
  const toForApi = decodedPhone.replace(/^\+/, "");
  const postDigits = decodedPhone.replace(/[^\d]/g, "");
  const postPhoneVariants = [...new Set([decodedPhone, postDigits, `+${postDigits}`])];
  const lastInbound = await prisma.inboundMessage.findFirst({
    where: { fromPhoneNumber: { in: postPhoneVariants } },
    orderBy: { receivedAt: "desc" },
    select: { orderId: true },
  });

  try {
    const result = await client.sendMedia(toForApi, mediaType, uploaded.id, {
      caption: caption || undefined,
      filename: mediaType === "document" ? filename : undefined,
    });

    // Record the outbound row so it shows in the thread alongside text.
    // We use a sentinel templateName of `<media>` plus the media id+caption
    // in templateVariablesJson so the UI knows how to render it.
    try {
      await prisma.whatsappMessage.create({
        data: {
          orderId: lastInbound?.orderId ?? undefined,
          phoneNumber: decodedPhone,
          phoneNumberId: client.getPhoneNumberId(),
          templateName: "<media>",
          templateLanguage: "",
          templateVariablesJson: {
            mediaType,
            mediaId: uploaded.id,
            mime: fileType,
            filename,
            caption: caption || null,
          },
          providerMessageId: result.messages?.[0]?.id ?? null,
          status: "SENT",
          sentBy: user.email,
          sentAt: new Date(),
        },
      });
    } catch {
      // ignore logging failure
    }

    return NextResponse.json({
      success: true,
      mediaId: uploaded.id,
      providerMessageId: result.messages?.[0]?.id ?? null,
    });
  } catch (err) {
    if (err instanceof WhatsAppApiError) {
      const inSessionExpired =
        err.metaCode === 131047 || err.metaCode === 131051;
      return NextResponse.json(
        {
          error: inSessionExpired
            ? "The 24-hour reply window has closed for this contact. Templates only."
            : err.message,
          meta: { code: err.metaCode, status: err.status },
        },
        { status: err.status }
      );
    }
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Send failed" },
      { status: 500 }
    );
  }
}
