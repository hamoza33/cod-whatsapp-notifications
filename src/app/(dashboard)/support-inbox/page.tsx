...
export default function SupportInboxPage() {
  ...
  const [selectedPhone, setSelectedPhone] = useState<string | null>(() => {
    ...
    return sessionStorage.getItem("support_inbox_selected_phone") || null;
  });
  ...
  const [replyText, setReplyText] = useState(() => {
    ...
    return sessionStorage.getItem("support_inbox_draft_text") || "";
  });
    ...
    async (phone: string) => {
      try {
        ...
        const data = await api.get<ThreadResponse>(
          `/whatsapp-support/inbox/${encodeURIComponent(phone)}${qs}`
        );
  ...
  useEffect(() => {
    async function loadNumbers() {
      try {
        const data = await api.get<{ numbers: WhatsappNumberOption[] }>("/whatsapp-support/numbers");
  ...
  useEffect(() => {
    if (selectedPhone) {
      sessionStorage.setItem("support_inbox_selected_phone", selectedPhone);
    } else {
      sessionStorage.removeItem("support_inbox_selected_phone");
    }
  }, [selectedPhone]);

  useEffect(() => {
    if (replyText) {
      sessionStorage.setItem("support_inbox_draft_text", replyText);
    } else {
      sessionStorage.removeItem("support_inbox_draft_text");
    }
  }, [replyText]);
  ...
  const handleReply = async () => {
    ...
    try {
      const response = await fetch(
        `/api/whatsapp-support/inbox/${encodeURIComponent(selectedPhone)}`,
        {
          method: "POST",
  ...
  const handleSendMedia = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/whatsapp-support/inbox/${encodeURIComponent(selectedPhone)}/media`,
  ...
  const fetchAiSuggestions = async (customContext?: string) => {
    ...
    try {
      const response = await fetch(
        `/api/whatsapp-support/inbox/${encodeURIComponent(selectedPhone)}/suggestions`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(
  ...
  const togglePin = async (phone: string, currentlyPinned: boolean) => {
    if (currentlyPinned) {
      await fetch(`/api/whatsapp-support/inbox/pin?phoneNumber=${encodeURIComponent(phone)}`, { method: "DELETE" });
    } else {
      await fetch("/api/whatsapp-support/inbox/pin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <div className="p-8 text-center">
              ...
              {!searchQuery && (
                <p className="mt-2 text-xs text-[#8696A0]">
                  ...
                  pointing at <code className="bg-[#F0F2F5] px-1 rounded">/api/whatsapp-support/webhook</code>
                </p>
              )}
            </div>
          )}
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (c.unreadCount > 0) {
                    ...
                    api.post("/whatsapp-support/inbox/read", { phoneNumber: c.phoneNumber }).catch(() => {});
                  }
                }}
              >
                {/* Avatar */}
...
function InboundBubble({
  ...
  const mediaUrl = msg.mediaId
    ? `/api/whatsapp-support/inbox/media/${encodeURIComponent(msg.mediaId)}`
    : null;
...
function OutboundBubble({
  ...
  const outboundMediaUrl = msg.outboundMedia
    ? `/api/whatsapp-support/inbox/media/${encodeURIComponent(msg.outboundMedia.mediaId)}`
    : null;
  const statusIcon = (() => {
    switch (msg.status) {
      case "READ":
